...
<div class="min-w-[4.5rem] bg-gray-50 dark:bg-gray-950 flex gap-2.5 flex-col pt-8">
	...
	<div class="flex justify-center relative group mt-2">
		{#if selected === 'learn'}
			<div class="absolute top-0 left-0 flex h-full">
				<div class="my-auto rounded-r-lg w-1 h-8 bg-black dark:bg-white"></div>
			</div>
		{/if}
		<Tooltip content="Learn" placement="right">
			<button
				class=" cursor-pointer {selected === 'learn'
					? 'rounded-2xl'
					: 'rounded-full'} p-2 hover:bg-gray-100 dark:hover:bg-gray-850 transition"
				on:click={() => {
					selected = 'learn';
					goto('/learn');
				}}
			>
				<BookOpen className="size-6 text-gray-700 dark:text-gray-100" />
			</button>
		</Tooltip>
	</div>
