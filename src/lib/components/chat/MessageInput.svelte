						...
						>
							...
							<div class=" flex justify-between mt-0.5 mb-2.5 mx-0.5 max-w-full" dir="ltr">
								...
								<div class="self-end flex space-x-1 mr-1 shrink-0 gap-[0.5px]">
									{#if (taskIds && taskIds.length > 0) || generating || (history.currentId && history.messages[history.currentId]?.role === 'assistant' && history.messages[history.currentId]?.done != true)}
									...
									{:else}
										...
										{#if (prompt === '' && files.length === 0 || $showCallOverlay || $config?.audio?.pipecat?.enabled) && ($_user?.role === 'admin' || ($_user?.permissions?.chat?.call ?? true))}
											<div class=" flex items-center">
												<!-- {$i18n.t('Call')} -->
												<Tooltip content={$showCallOverlay ? $i18n.t('End') : $i18n.t('Talk')}>
													<button
														class="{$showCallOverlay
															? 'bg-red-500 text-white hover:bg-red-600'
															: 'mira-gradient text-white hover:opacity-90 mira-glow'
														} transition rounded-full p-1.5 self-center"
														type="button"
														on:click={async () => {
															try {
																// Toggle: if already in voice mode, close it
																if ($showCallOverlay) {
													...
													aria-label={$showCallOverlay ? $i18n.t('End') : $i18n.t('Talk')}
												>
													{#if $showCallOverlay}
