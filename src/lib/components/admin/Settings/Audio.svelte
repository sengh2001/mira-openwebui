<script lang="ts">
	...
	// Pipecat
	let PIPECAT_ENABLED = false;
	let PIPECAT_API_URL = '';
	let PIPECAT_CONNECTION_MODE = 'webrtc';
	...
	const updateConfigHandler = async () => {
		...
		const res = await updateAudioConfig(localStorage.token, {
			...
			pipecat: {
				PIPECAT_ENABLED: PIPECAT_ENABLED,
				PIPECAT_API_URL: PIPECAT_API_URL,
				PIPECAT_CONNECTION_MODE: PIPECAT_CONNECTION_MODE
			}
		});
	...
	onMount(async () => {
		...
		if (res) {
			...
			PIPECAT_ENABLED = res.pipecat?.PIPECAT_ENABLED || false;
			PIPECAT_API_URL = res.pipecat?.PIPECAT_API_URL || 'http://localhost:7860';
			PIPECAT_CONNECTION_MODE = res.pipecat?.PIPECAT_CONNECTION_MODE || 'webrtc';
		}
...
>
	<div class=" space-y-3 overflow-y-scroll scrollbar-hidden h-full">
		<div class="flex flex-col gap-3">
			<div>
				<div class=" mb-2.5 text-sm font-medium">Pipecat Voice (Beta)</div>
				<hr class=" border-gray-100/30 dark:border-gray-850/30 my-2" />

				<div class=" py-0.5 flex w-full justify-between">
					<div class=" self-center text-xs font-medium">Enable Pipecat Voice Integration</div>
					<div class="mt-1">
						<Switch bind:state={PIPECAT_ENABLED} />
					</div>
				</div>

				{#if PIPECAT_ENABLED}
					<div class=" py-0.5 flex w-full justify-between">
						<div class=" self-center text-xs font-medium">Connection Mode</div>
						<div class="flex items-center relative">
							<select
								...
								bind:value={PIPECAT_CONNECTION_MODE}
							>
					...
					<div class=" py-0.5 flex w-full justify-between">
						<div class=" self-center text-xs font-medium">Pipecat Server URL</div>
						<div class="flex items-center relative text-xs">
							<input
								type="text"
								bind:value={PIPECAT_API_URL}
								placeholder="http://localhost:7860"
								class=" text-sm text-right bg-transparent dark:text-gray-300 outline-hidden w-64"
							/>
						</div>
					</div>
				{/if}
